    ...
    <View style={styles.container}>
      <View style={styles.levelRow}>
        {([1, 2, 3] as const).map(lv => (
          ...
          >
            <Text style={[styles.levelBtnText, currentLevel === lv && { color: levelColors[lv] }]}>
              {levelLabels[lv]}
            </Text>
          </TouchableOpacity>
        ))}
      ...
      {hint ? (
        <View style={[styles.hintBox, { backgroundColor: levelBg[hint.level] }]}>
          <View style={styles.hintHeader}>
            ...
            <Text style={[styles.levelBadge, { color: levelColors[hint.level] }]}>
              Livello {hint.level}
            </Text>
          </View>
        ...
        <View style={styles.emptyHint}>
          <Text style={styles.emptyHintText}>Premi "Chiedi indizio" per ricevere aiuto</Text>
        </View>
      )}
...
const styles = StyleSheet.create({
  ...
  levelBtnText: { fontSize: 12, color: '#5F5E5A', fontWeight: '500' },
  hintBox: { borderRadius: 10, padding: 12, marginBottom: 12 },
  hintHeader: {
    flexDirection: 'row',
    alignItems: 'center',
  ...
  levelBadge: { fontSize: 11, paddingHorizontal: 8, paddingVertical: 2 },
  hintText: { fontSize: 13, color: '#444441', lineHeight: 19 },
  emptyHint: {
    padding: 16,
    borderRadius: 10,
  ...
  emptyHintText: { fontSize: 13, color: '#888780', textAlign: 'center' },
  hintButton: {
  ...
  hintButtonText: { color: '#CECBF6', fontSize: 15, fontWeight: '600' },
  hintCount: { fontSize: 12, color: '#888780', textAlign: 'center', marginTop: 8 },
});
