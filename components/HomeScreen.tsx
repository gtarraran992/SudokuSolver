...
export type RootStackParamList = {
  Home: undefined;
  Hint: { board: BoardState; solution: Grid };
};
...
export default function HomeScreen({ navigation }: Props) {
  ...
  const handleNumberPress = useCallback((num: CellValue) => {
    ...
    setBoard(prev => {
      ...
      next[row][col] = { value: num, type: inputMode, isError: false };
      return next;
    });
  ...
  const handleCalculate = useCallback(() => {
    ...
    if (!validation.valid) {
      ...
      Alert.alert('Griglia non valida', `Trovati ${validation.errors.length} conflitti. Controlla le celle rosse.`);
      return;
    }

    const solution = solve(grid);
    if (!solution) {
      Alert.alert('Nessuna soluzione', 'Questa griglia non ha soluzione. Verifica i numeri inseriti.');
      return;
    }

    navigation.navigate('Hint', { board, solution });
  }, [board, navigation]);
      ...
      <ScrollView contentContainerStyle={styles.scroll}>
        ...
        <View style={styles.modeRow}>
          <TouchableOpacity
            ...
            <Text style={[styles.modeBtnText, inputMode === 'given' && styles.modeBtnTextActive]}>Numeri dati</Text>
          </TouchableOpacity>
          <TouchableOpacity
            style={[styles.modeBtn, inputMode === 'user' && styles.modeBtnActiveUser]}
            onPress={() => setInputMode('user')}>
            <Text style={[styles.modeBtnText, inputMode === 'user' && styles.modeBtnTextActiveUser]}>Già risolti</Text>
          </TouchableOpacity>
        </View>
...
const styles = StyleSheet.create({
  ...
  modeBtnActive: { backgroundColor: '#F1EFE8', borderColor: '#5F5E5A' },
  modeBtnActiveUser: { backgroundColor: '#E6F1FB', borderColor: '#185FA5' },
  ...
  calcBtn: { backgroundColor: '#3C3489', borderRadius: 12, paddingVertical: 15, alignItems: 'center', marginBottom: 10 },
